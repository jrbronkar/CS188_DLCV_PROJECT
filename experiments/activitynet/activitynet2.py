...
class ActivityNet(data.Dataset):
    ...
    def __init__(self, split):
        ...
        self.vid_to_idx = {}
        self.features = None
        with h5py.File(os.path.join(self.data_dir,'ImageNetShuffle2016_features.h5'),'r') as f:
            self.features = torch.from_numpy(f["features"][:])
            pca = PCA(500)
            self.features = pca.fit_transform(self.features)
        self.features = torch.tensor(self.features)
        with open('/mnt/disks/disk-3/VideoX/2D-TAN/data/ActivityNet/ImageNetShuffle2016_quids.txt') as f:
            i = 0
            for l in f:
                s = l.split(',')
                self.vid_to_idx[s[2][:-4]] = i
                i+=1
    ...
    def get_video_features(self, vid):
        ...
        features = self.features[self.vid_to_idx[vid]]
